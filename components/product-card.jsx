...
export function ProductCard({ id, title, price, discountPrice, image, category, rating, badge }) {
  const { toast } = useToast()
  const { addToCart } = useCart()
  const { addToWishlist, isInWishlist, removeFromWishlist } = useWishlist()
  const [isWishlisted, setIsWishlisted] = useState(isInWishlist ? isInWishlist(id) : false)
      ...
      <Link href={`/products/${id}`}>
        ...
        <div className="pt-3">
          ...
          <div className="mt-1 flex items-center gap-2">
            <p className="font-semibold">₹{(discountPrice || price).toLocaleString()}</p>
            {discountPrice && <p className="text-sm text-muted-foreground line-through">₹{price.toLocaleString()}</p>}
          </div>
        </div>
      </Link>
      <div className="mt-3">
